...
{
#ifdef _WIN32
	// Fast-path: if events are already queued, do not block.
	{
	...
	std::array<HANDLE, 2> handles = { _interrupt, _consoleIn };
	...
	while (true)
	{
		DWORD event = WaitForMultipleObjects(
			...
			handles.data(),
			FALSE,
			timeout_ > 0 ? timeout_ : INFINITE
		);

		switch (event)
		{
		case (WAIT_OBJECT_0 + 0): // _interrupt
		{
			std::lock_guard<std::mutex> Lock(_eventsMutex);
			...
			// IMPORTANT: only reset when queue is empty.
			if (_events.empty())
			{
				ResetEvent(_interrupt);
			}
		...
		case (WAIT_OBJECT_0 + 1): // _consoleIn
		{
			INPUT_RECORD rec;
			DWORD count = 0;
...
{
#ifdef _WIN32
	bool bNeedSignal = false;

	{
		std::lock_guard<std::mutex> Lock(_eventsMutex);
		bNeedSignal = _events.empty();
