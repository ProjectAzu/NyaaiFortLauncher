...
type ParsedProp = {
  ...
  valueRange: vscode.Range; // inside the outermost {...} (same as CapturedValue in C++)
};
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument(doc => {
      if (doc.languageId === LANG_ID) refreshDiagnostics(doc, diagnosticCollection);
    }),
    vscode.workspace.onDidChangeTextDocument(e => {
      if (e.document.languageId === LANG_ID) refreshDiagnostics(e.document, diagnosticCollection);
    }),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nfortconfig.formatDocument", async () => {
 ...
 */
function parsePropertiesSetDataLikeCpp(
  text: string,
  document: vscode.TextDocument
): { props: ParsedProp[]; errors: ParseError[] } {
  const props: ParsedProp[] = [];
  const errors: ParseError[] = [];

  const isWs = (ch: string) => /\s/.test(ch); // close enough to iswspace for editor use
  ...
  const cleanNameCppStyle = (raw: string): string => {
  ...
  while (true) {
    ...
    if (index >= text.length || text[index] !== "{") {
      const range = new vscode.Range(
        ...
        document.positionAt(Math.min(index + 1, text.length))
      );
      errors.push({
    ...
    if (braceDepth !== 0) {
      ...
      const range = new vscode.Range(
        document.positionAt(Math.max(valueStart - 1, 0)),
        document.positionAt(text.length)
      );
      errors.push({
    ...
    const valueRange = new vscode.Range(
      ...
      document.positionAt(valueEndExclusive)
    );
 ...
 */
function formatTopLevelProps(input: string): string {
  ...
  const cleanName = (raw: string): string => raw.replace(/[^\p{L}\p{N}\p{P}]+/gu, "");
